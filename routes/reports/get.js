...
export default async (req, res) => {
  ...
  .where('l.type', 'like', `%${String(type).toUpperCase()}%`).where('l.fromDevice', device).where('d.site_id', 'like', `%${site}%`).where('d.block_id', 'like', `%${block}%`).where('d.floor_id', 'like', `%${floor}%`).where('d.company_id', 'like', `%${comp}%`).whereRaw('date_format(l.detectionTime, \'%Y-%m-%d %H:%i\') >= cast(\''+sd===''?moment().subtract(7, 'days').format('YYYY-MM-DD 00:00'):sd+'\' as date)').whereRaw('date_format(l.detectionTime, \'%Y-%m-%d %H:%i\') <= cast(\''+ed===''?moment().format('YYYY-MM-DD 00:00'):ed+'\' as date)').orderBy('l.detectionTime', 'desc').offset(offset);
  //whereRaw('date_format(detectionTime, \'%Y-%m-%d  %H:%i\') between cast(\''+sd.toString()+'\' as date) and cast(\''+ed.toString()+'\' as date)')
