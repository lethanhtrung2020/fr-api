...
export default async (req, res) => {
  ...
  .where('l.type', 'like', `%${String(type).toUpperCase()}%`).where('l.fromDevice', device).where('d.site_id', 'like', `%${site}%`).where('d.block_id', 'like', `%${block}%`).where('d.floor_id', 'like', `%${floor}%`).where('d.company_id', 'like', `%${comp}%`).where(knex.raw('?', new Date('l.detectionTime').toDateString()), '=', knex.raw('?', sd)).orderBy('l.detectionTime', 'desc').offset(offset);
