...
export default async (req, res) => {
  ...
  console.log('sd: ' +  (sd === '' ? moment() : moment(sd).format('mm/dd/yyyy hh:mm')) + ' -  ed: ' +  (ed === '' ? moment() : moment(ed).format('mm/dd/yyyy hh:mm')));
  // , \'%Y-%m-%d\'
  ...
  .from("detection_logs as l").whereRaw('moment(l.detectionTime).format(\'mm/dd/yyyy hh:mm\') BETWEEN ? AND ?', [moment(sd).format('mm/dd/yyyy hh:mm'), moment(ed).format('mm/dd/yyyy hh:mm')])
